...
export function useAllIncentives(): {
  ...
  const { loading: loading, data: subgraphIncentives } =
    useAllIncentivesSubgraph();
  ...
  const incentiveStates = useSingleContractMultipleData(
    staker,
    "incentives",
    incentiveIds
  );
  ...
  return useMemo(() => {
    if (
      ...
      !pools ||
      !subgraphIncentives ||
      incentiveStates.some((s) => s.loading)
    )
      return { loading: true };
      ...
      incentives: subgraphIncentives
        .map((result: any, ix: number): Incentive | null => {
          let token = new Token(
            ...
            result.rewardToken,
            18,
            result.rewardToken.slice(0, 6) +
              "..." +
              result.rewardToken.slice(-4)
          );
          ...
          const initialRewardAmount = CurrencyAmount.fromRawAmount(
            token,
            result.reward
          );
          const rewardAmountRemaining = CurrencyAmount.fromRawAmount(
            token,
            state.totalRewardUnclaimed
          );

          const [startTime, endTime] = [
            parseInt(result.startTime),
            parseInt(result.endTime),
          ];

          const rewardRatePerSecond = initialRewardAmount.divide(
            endTime - startTime
          );
