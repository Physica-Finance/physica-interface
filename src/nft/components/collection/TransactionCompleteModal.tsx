...
const TxCompleteModal = () => {
  ...
  const setTransactionResponse = useTransactionResponse(
    (state) => state.setTransactionResponse
  );
  const isMobile = useIsMobile();
  const txHashUrl = getExplorerLink(1, txHash, ExplorerDataType.TRANSACTION);
  const shouldShowModal =
    (txState === TxStateType.Success || txState === TxStateType.Failed) &&
    txState;
            ...
            {showPurchasedModal && (
              <Trace
                ...
                properties={{
                  ...
                  usd_value:
                    parseFloat(formatEther(totalPurchaseValue)) * ethPrice,
              ...
              >
                <Box className={styles.successModal} onClick={stopPropagation}>
                    ...
                    {[...nftsPurchased].map((nft, index) => (
                      <img
                        className={clsx(
                          ...
                          nftsPurchased.length > 1 &&
                            styles.successAssetImageGrid
                        )}
                        style={{
                          maxHeight: `${getSuccessfulImageSize(
                            nftsPurchased.length,
                            isMobile
                          )}px`,
                          maxWidth: `${getSuccessfulImageSize(
                            nftsPurchased.length,
                            isMobile
                          )}px`,
                        }}
                  ...
                  {nftsPurchased.length > 32 && (
                    <Box className={styles.overflowFade} />
                  )}
                  <Box
                    display="flex"
                    width="full"
                  ...
                  >
                    <Row>
                      ...
                      <Box>
                        {formatEthPrice(totalPurchaseValue.toString())} ETH
                      </Box>
                    </Row>
                    <a
                      href={txHashUrl}
                ...
                >
                  ...
                  >
                    ...
                    <p className={styles.bodySmall}>
                      ...
                      {formatUsdPrice(txFeeFiat)} was used for gas in attempt to
                      complete this transaction. For support, please visit our{" "}
                    ...
                    <Box className={styles.allUnavailableAssets}>
                      {nftsNotPurchased.length >= 3 && (
                        ...
                        >
                          ...
                          <Box
                            color={
                              showUnavailable ? "textPrimary" : "textSecondary"
                            }
                          ...
                          <ChevronUpIcon
                            className={`${
                              !showUnavailable && styles.chevronDown
                            } ${styles.chevron}`}
                          />
                        </Box>
                      )}
                    ...
                    {showUnavailable && (
                      <Box className={styles.fullRefundOverflowFade} />
                    )}
                    <p
                      className={styles.totalEthCost}
                      style={{ marginBottom: "2px" }}
                    ...
                    >
                      <a href={txHashUrl} target="_blank" rel="noreferrer">
                        <Box className={styles.addressHash}>
                          View on Etherscan
                        </Box>
                      </a>
                    </Box>
