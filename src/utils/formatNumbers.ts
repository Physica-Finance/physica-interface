...
export const formatTransactionAmountPrecise = (num: number | undefined | null, maxDigits = 9) => {
  if (num === 0) return '0.00'
  if (!num) return ''
  if (num < 0.0000001) {
    return '<0.000001'
  }
  if (num >= 0.0000001 && num < 1) {
    return `${Number(num.toFixed(8)).toLocaleString(DEFAULT_LOCALE, {
      minimumFractionDigits: 2,
