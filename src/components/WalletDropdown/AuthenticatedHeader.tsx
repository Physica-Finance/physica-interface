...
const AuthenticatedHeader = () => {
  ...
  const animateBuyCryptoButtonBorder =
    acknowledgements?.user && !acknowledgements.system;
  useEffect(() => {
    let stale = false;
    let timeoutId = 0;
    ...
    <>
      <HeaderWrapper>
        <FlexContainer>
          ...
          {ENSName ? (
            ...
            <ThemedText.SubHeader marginTop="2.5px">
              {account && shortenAddress(account, 2, 4)}
      ...
      <Column>
        <BalanceWrapper>
          ...
          {amountUSD !== undefined && (
            <USDText>{formatUSDPrice(amountUSD)} USD</USDText>
          )}
        </BalanceWrapper>
        {/*<ProfileButton
        ...
        {/*isClaimAvailable && (
          <UNIButton size={ButtonSize.medium} emphasis={ButtonEmphasis.medium} onClick={openNftModal}>
            <Trans>Claim Physica NFT Airdrop</Trans>
          </UNIButton>
        )*/}
      </Column>
    </>
