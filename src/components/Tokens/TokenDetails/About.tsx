...
const truncateDescription = (desc: string) => {
  ...
  tokenDescriptionTruncated = `${tokenDescriptionTruncated.slice(
    0,
    Math.min(
      tokenDescriptionTruncated.length,
      tokenDescriptionTruncated.lastIndexOf(" ")
    )
  )}...`;
    ...
    <AboutContainer data-testid="token-details-about-section">
      ...
      <TokenDescriptionContainer>
        ...
        {shouldTruncate && (
          <TruncateDescriptionButton
            onClick={() => setIsDescriptionTruncated(!isDescriptionTruncated)}
          >
      ...
      <ResourcesContainer data-cy="resources-container">
        <Resource
          ...
          link={`${baseExplorerUrl}${
            address === "NATIVE" ? "" : "address/" + address
          }`}
        />
        <Resource
          name="More analytics"
          link={`https://info.uniswap.org/#/tokens/${address}`}
        />
        {homepageUrl && <Resource name="Website" link={homepageUrl} />}
        {twitterName && (
          <Resource
            name="Twitter"
            link={`https://twitter.com/${twitterName}`}
          />
        )}
      </ResourcesContainer>
    </AboutContainer>
