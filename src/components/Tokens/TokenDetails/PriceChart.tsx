...
function fixChart(prices: PricePoint[] | undefined | null) {
  ...
  if (lastValue && lastValue !== prices[prices.length - 1]) {
    blanks.push([
      lastValue,
      { ...prices[prices.length - 1], value: lastValue.value },
    ]);
  }
...
export function PriceChart({
  ...
  const missingPricesMessage = !chartAvailable ? (
    prices?.length === 0 ? (
      <>
        <Trans>
          Missing price data due to recently low trading volume on Physica v3
        </Trans>
      </>
    ) : (
  ...
  const graphHeight =
    height - timeOptionsHeight > 0 ? height - timeOptionsHeight : 0;
  const graphInnerHeight =
    graphHeight - margin.top - margin.bottom > 0
  ...
  function tickFormat(
    ...
    const offsetTime =
      (endingPrice.timestamp.valueOf() - startingPrice.timestamp.valueOf()) /
      24;
    const startDateWithOffset = new Date(
      (startingPrice.timestamp.valueOf() + offsetTime) * 1000
    );
    const endDateWithOffset = new Date(
      (endingPrice.timestamp.valueOf() - offsetTime) * 1000
    );
    switch (timePeriod) {
      ...
      case TimePeriod.DAY:
        return [
          ...
          dayHourFormatter(locale),
          timeHour
            .range(startDateWithOffset, endDateWithOffset, 4)
            .map((x) => x.valueOf() / 1000),
        ];
      case TimePeriod.WEEK:
        return [
          ...
          dayHourFormatter(locale),
          timeDay
            .range(startDateWithOffset, endDateWithOffset, 1)
            .map((x) => x.valueOf() / 1000),
        ];
      case TimePeriod.MONTH:
        return [
          ...
          dayHourFormatter(locale),
          timeDay
            .range(startDateWithOffset, endDateWithOffset, 7)
            .map((x) => x.valueOf() / 1000),
        ];
