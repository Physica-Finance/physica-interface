...
function useOnChainToken(address: string | undefined, skip: boolean) {
  const token = useTokenFromActiveNetwork(
    skip || !address ? undefined : address
  );
...
export default function TokenDetails({
  ...
  if (!urlAddress) {
    throw new Error(
      "Invalid token details route: tokenAddress param is undefined"
    );
  }
  const address = useMemo(
    () =>
      urlAddress === NATIVE_CHAIN_ID
        ? urlAddress
        : isAddress(urlAddress) || undefined,
    [urlAddress]
  );
  ...
  const { token, didFetchFromChain } = useRelevantToken(
    address,
    pageChainId,
    tokenQueryData
  );
    ...
    (update: Chain) => {
      ...
      if (bridgedAddress) {
        startTokenTransition(() =>
          navigate(getTokenDetailsURL({ address: bridgedAddress, chain }))
        );
      } else if (didFetchFromChain || token?.isNative) {
        startTokenTransition(() =>
          navigate(getTokenDetailsURL({ address, chain }))
        );
      }
    },
    [
  ...
  const shouldShowSpeedbump =
    !useIsUserAddedTokenOnChain(address, pageChainId) && tokenWarning !== null;
  ...
  if (token === undefined || !address) {
    return (
      <InvalidTokenDetails
        chainName={address && getChainInfo(pageChainId)?.label}
      />
    );
  }
  return (
    ...
    >
      <TokenDetailsLayout>
        {token && !isPending ? (
          <LeftPanel>
            <BreadcrumbNavLink to={`/tokens/${chain.toLowerCase()}`}>
              <ArrowLeft data-testid="token-details-return-button" size={14} />{" "}
              Tokens
            </BreadcrumbNavLink>
        ...
        >
          ...
          {tokenWarning && (
            <TokenSafetyMessage tokenAddress={address} warning={tokenWarning} />
          )}
